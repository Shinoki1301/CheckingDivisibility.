...
// Структура, описывающая один маршрут: путь и связанный с ним идентификатор
// маршрута
typedef struct {
...
Route resolve_route(struct mg_http_message* hm) {
    ...
    size_t count = sizeof (route_table) /
	           sizeof (route_table[0]);
    for (size_t iter = 0; iter < count; ++iter) {
        // Сравнение URI запроса с каждым путем в таблице
        if (mg_strcmp(hm->uri, mg_str (route_table[iter].path)) == 0) {
            resolved = route_table[iter].route;
            iter     = count;
        }
    }
